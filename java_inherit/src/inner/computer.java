...
// static 클래스 - 내부 static클래스에는 인스턴스 변수 , 메서드 와 static 변수 , 메서드 
//				   모두 선언이 가능하다.
//				 - 외부 클래스의 인스턴스 변수,메서드의 사용은 불가능 하다.
//				 - 외부 클래스의 객체를 통한 객체 생성이 아니라 독자적으로 생성 가능
//				 - static 변수 , 메서드는 메모리에 하나만 올라가지만 
//				   static 클래스는 여러개 생성이 가능하다.
//				 - 외부 클래스의 인스터스를 사용하지 않는 내부 클래스라면
//				 - 내부클래스를 static 으로 선언한다.(일부 IDE에서는 경고뜬다)

// 로컬 클래스 - 메서드 내부에 위치하는 클래스
//			   - 메서드 내부에서만 사용가능
//			   - 접근제어자 , static 을 붙일 수 없다.
