...
public class test1 {
	...
	public static void main(String[] args) {
		...
		DoubleStream floatStream = DoubleStream.iterate(1,n->n+0.1).limit(10);
		floatStream.forEach(System.out::println);
		DoubleStream floatStream2 = new Random().doubles(5);
		floatStream2.forEach(System.out::println);
		...
		//filter - 참과 거짓에 따라 동작 , if문의 조건식과 비슷한 식을 구사하면된다. 
...
// stream - 데이터의 흐름 , 컬렉션 프레임워크 여러개의 메서드를 조합하여
// 원하는 결과를 얻을 수 있는데 stream을 이용하면 코드의 양을 줄이고 간단하게 표현할 수 있다.
// 배열과 컬렉션을 함수형이라는 형태로 처리가 가능하다.
// stream은 병렬처리가 가능하다 . 둘이상의 작업을 동시에 진행 가능 ( 쓰레드 이용 )
// stream 사용 : (배열또는컬렉션).맵핑.필터링.결과만들기 -> 결과물
// stream 사용하지 않으면 맵핑작업을 위한 메서드를 통해 반환을 받아 필터링을 위한 메서드에 
